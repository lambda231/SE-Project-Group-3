...
?>
<iframe src="PrintDocumentRedirectPage.php" id="subscreen" allowtransparency=True style="border:none; border: 1px solid #000000; position:absolute; z-index:1" width=0% height=0% >
</iframe>

<line> 
	<displaybox><img src="icon_print.png" width=50 height=50></displaybox>
	<displaybox0><b> SỬ DỤNG MÁY IN</b></displaybox0>
	<displaypage id="RemainingPage"></displaypage>
	<displaybuybox type="button" onclick="location.href='bkpay.php'">
		<img src="icon_buy.png" width=50 height=50>
		<displaybuy><b> Mua trang </b></displaybuy>
	</displaybuybox>
</line>
...
<but><input type="button" name="back" value="Trở lại" onclick="location.href='LogView(SPSO).php'"></but>
...
function displayTable(part){
	...
	for (i = row_per_page*part; i < Math.min(row_per_page*(part+1), filterlogs.length); i++){
		...
		row.addEventListener("click", function(){
			...
			remainpage_ele.innerHTML = "(Số trang còn lại: " + remaining_pages + ")<br>"
			}
		)
...
function displayRemainPage(){
	document.getElementById("RemainingPage").innerHTML = "<b>Số trang còn lại: " + remaining_pages + "</b>";
}
